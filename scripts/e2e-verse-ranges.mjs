...
if (await page.locator('.mindmap-node.node-reference:has-text("Quran 2:10")').count()) throw new Error("Overlapping verse was duplicated outside its collection.");
...
await page.waitForFunction(() => document.querySelectorAll('.mindmap-collection-panel:not([hidden]) [data-collection-reference]').length === 34);
if (await page.locator('.mindmap-collection-panel [data-collection-reference]').count() !== 34) throw new Error("Mind map collection did not expand to its verse tray.");
if (await page.locator('.mindmap-collection-panel [data-collection-reference="2:10"]').count() !== 1) throw new Error("Shared verse was not unique inside the expanded collection.");
await page.screenshot({ path: "/tmp/abrahamic-mindmap-collection.png", fullPage: true });
await page.locator('.mindmap-node.node-collection:has-text("2:1-34")').click();
await page.waitForFunction(() => document.querySelector('.mindmap-collection-panel')?.hidden);
